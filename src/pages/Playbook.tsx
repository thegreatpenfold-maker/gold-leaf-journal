...
export default function Playbook() {
  ...
  const handleSave = () => {
    ...
    if (editing) {
      updatePlaybook({ id: editing, ...form, rules: form.rules.filter(r => r.trim()) });
      setEditing(null);
    } else {
      addPlaybook({ id: crypto.randomUUID(), ...form, rules: form.rules.filter(r => r.trim()) });
    }
    setShowForm(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setShowForm(true); setEditing(null); setForm({ name: '', description: '', rules: [''], conditions: '', strategyTag: STRATEGIES[0] }); }} className="flex items-center gap-2 px-4 py-2 rounded-lg gold-gradient text-primary-foreground font-semibold text-sm">
      ...
      {showForm && (
        <div className="glass-card rounded-xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-semibold">{editing ? 'Edit' : 'New'} Strategy</h3>
          ...
          <div>
            <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2 block">Rules</label>
            {form.rules.map((r, i) => (
          ...
          <select value={form.strategyTag} onChange={e => setForm(f => ({ ...f, strategyTag: e.target.value }))} className={inputCls}>
            {STRATEGIES.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          <button onClick={handleSave} className="flex items-center gap-2 px-4 py-2 rounded-lg gold-gradient text-primary-foreground font-semibold text-sm"><Save size={14} /> Save</button>
        </div>
      )}
            ...
            <div key={p.id} className="glass-card rounded-xl p-5 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => editEntry(p.id)} className="text-muted-foreground hover:text-primary"><Edit2 size={14} /></button>
                  <button onClick={() => { deletePlaybook(p.id); toast.success('Deleted'); }} className="text-muted-foreground hover:text-destructive"><Trash2 size={14} /></button>
                </div>
              </div>
              <p className="text-sm text-muted-foreground">{p.description}</p>
              <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 rounded-full bg-primary/10 text-primary text-xs font-medium">{p.strategyTag}</span>
                {wr !== null && <span className={`text-xs font-bold ${parseFloat(wr) >= 50 ? 'text-success' : 'text-destructive'}`}>{wr}% WR</span>}
              </div>
              {p.rules.length > 0 && (
